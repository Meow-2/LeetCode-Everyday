...
public:
    // 深拷贝
    // void backtrack(TreeNode* cur, TreeNode* p, vector<TreeNode*>& path, vector<TreeNode*>&
    // p_path)
    // {
    //     if (!cur)
    //         return;
    ...
    bool      dfs(TreeNode* root, TreeNode* p, TreeNode* q)
    {
        ...
        if (root == q || root == p)
            flag = true;
        bool isLeftHas  = dfs(root->left, p, q);
        bool isRightHas = dfs(root->right, p, q);
        if (isLeftHas && isRightHas || (isLeftHas || isRightHas) && flag)
            res = root;
        return isLeftHas || isRightHas || flag;
    }
    TreeNode* lowestCommonAncestor(TreeNode* root, TreeNode* p, TreeNode* q)
    {
        dfs(root, p, q);
